...
it('tests initialize - configs properties exist , this.config.datastores.length is strictly equal to 0 and callback is called.', async () => {
  ...
  new DataService({
    logger: {
      createLogger: test.sinon.stub().returns({ createLogger: stubCreateLogger }),
    },
  });
...
it('tests parseFields - value is null. ', async () => {
  const dataService = new DataService();
  const mockCallbackOne = test.sinon.stub();
  const mockFields = null;
  const stubInitialize = test.sinon
...
it('tests parseFields - expression is an array and is not a string. It then calls this.update. ', async () => {
  const dataService = new DataService();
  const mockCallbackOne = test.sinon.stub();
  const mockFields = { $regex: [] };
  const stubInitialize = test.sinon
    .stub(HappnDbProviderLoki.prototype, 'initialize')
    .callsFake((cb) => {
      cb(null);
  ...
  test.chai.expect(result).to.eql({ $regex: /(?:)/ });
...
it('tests compact method - interval and dataStoreKey are functions. It then calls this.compact and calls provider.compact.', async () => {
  ...
  test.chai.expect(stubCompacting).to.have.been.calledWithExactly(test.sinon.match.func);
  test.chai
    .expect(stubHandleSystem)
    .to.have.been.calledWithExactly(
      test.sinon.match.instanceOf(Error).and(test.sinon.match.has('message', 'mockError')),
...
it('tests compact method - callback is called if interval is falsy and this.__providerHasFeature returns truthy. this.errorService.handleSystem was not called.', async () => {
  ...
  dataService.happn = mockHappn;
  mockConfig.secure = null;
  mockConfig.datastores = [{ provider: 'happn-db-provider-nedb', name: 'nedb', patterns: [] }];
...
it('tests addDataProviderPatterns method', async () => {
  ...
  test.chai
    ...
    .to.have.been.calledWithExactly('mockPatternOne', 'mockRoute', 'DATASTORE_ROUTES', 0, true);
  test.chai.expect(dataService.dataroutessorted).to.eql(['mockPatternOne', 'mockPatternTwo']);
